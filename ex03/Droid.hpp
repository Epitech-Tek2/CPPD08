...
#define TAKE_ENERGY_CAR(tmp, this, energy) ({tmp = std::min(600 - this->_energy, energy); this->_energy += tmp; energy -= tmp;})
#define OPERATOR_PROC(tmp, status, r, exp) ({ \
    *tmp += status; \
    setStatus(tmp); \
    _battleData->addExp(exp); \
    return (r); \
})
...
class Droid
{
    public:
        ...
        bool operator==(Droid const& droid) const noexcept;
        bool operator()(std::string const *task, size_t exp) noexcept;
